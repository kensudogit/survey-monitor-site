...
class TestDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $surveys = $this->generateSurveys(15);
        $this->generateResponses($surveys, $users, 1500);
    ...
    {
        $this->command->info('📂 Generating survey categories...');
        ...
        SurveyCategory::insert($categories);
    }

    private function generateUsers($count)
    {
        $this->command->info("👥 Generating {$count} test users...");
    ...
    {
        $this->command->info("📊 Generating {$count} test surveys...");
    ...
    private function generateResponses($surveys, $users, $targetCount)
    {
        $this->command->info("📝 Generating {$targetCount} test responses...");
        ...
        foreach ($surveys as $survey) {
            ...
            foreach ($responseUsers as $user) {
                if ($responseCount >= $targetCount) break;
    ...
    {
        $this->command->info('🤖 Generating AI insights...');
    ...
    private function displaySummary()
    {
        ...
        $this->command->line('  - Categories: ' . SurveyCategory::count());
        $this->command->line('  - Surveys: ' . Survey::count());
        $this->command->line('  - Users: ' . User::count());
        $this->command->line('  - Responses: ' . SurveyResponse::count());
        $this->command->line('  - Analytics Records: ' . SurveyAnalytics::count());
        $this->command->line('  - AI Insights: ' . SurveyInsight::count());
        
        $this->command->info('🎯 AI Analytics Features Demonstrated:');
        $this->command->line('  ✅ Real-time data processing');
        $this->command->line('  ✅ Sentiment analysis (Japanese)');
        $this->command->line('  ✅ Quality scoring');
        $this->command->line('  ✅ Demographic analysis');
        $this->command->line('  ✅ Trend analysis');
        $this->command->line('  ✅ AI-powered insights');
        $this->command->line('  ✅ Completion rate analysis');
        $this->command->line('  ✅ Response pattern detection');
    }
}
